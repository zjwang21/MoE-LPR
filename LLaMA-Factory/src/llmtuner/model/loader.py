...
def load_model_and_tokenizer(
    ...
    if finetuning_args.finetuning_type in ['moea', 'ca']:
        model = load_merge_models(finetuning_args, model_args)
        model.train()
    ...
    if model is None:
        if model_args.moe_with_aux:
            from transformers.models.qwen2 import Qwen2ForCausalMoeLM
            model = Qwen2ForCausalMoeLM.from_pretrained(
                model_args.model_name_or_path,
                config=config,
    ...
    #if model_args.optimize_embedding:
    #    print("Making embedding and lm_head trainable.", flush=True)
